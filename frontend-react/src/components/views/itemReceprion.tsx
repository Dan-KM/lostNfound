...
export const ItemReception = () => {
    ...
    async function getItems() {
        try {
            const res = await API.get('inventory/user-item/?item_type=found&status=submitted');
    ...
    async function handleReceiveItem(id: number) {
        try {
            const resp = await API.patch(
                `inventory/user-item/${id}/update-status/`,
                { status: 'in office' }
            );

            console.log("Status updated:", resp.data);

            // Optional: refresh the list after update
            getItems(); // assuming you have this function defined
        } catch (error) {
  ...
  return (
    <ResponsiveShadcnTable userItems = {userItems} handleReceiveItem={handleReceiveItem}/>
  )
}
...
interface ResponsiveShadcnTableProps {
  userItems?: UserItem[];
  handleReceiveItem: (id: number) => Promise<void>;
}
  ...
  return (
    <div className="w-full p-4">
      ...
      <div className="md:hidden space-y-4">
        {propUserItems && propUserItems.map((item: UserItem) => (
